...
namespace electronic_library_6.Infrastructure
{
    ...
    IRepository<Role> rolesRepository)
        ...
        private string GetSalt () => DateTime.UtcNow.ToString() + DateTime.Now.Ticks;
        ...
        public async Task<User> RegistrationAsync (string fullname, string username, string password)
        {
            ...
            Role? clientRole = (await roles.FindWhere(r => r.Name == "JuniorAdmin")).FirstOrDefault();

            if (clientRole is null)
                throw new InvalidOperationException("Role 'JuniorAdmin' not found in database");
